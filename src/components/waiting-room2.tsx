...
export default class WaitingRoom2 extends React.Component<any, any> {
  constructor(props) {
    super(props);
  }
  ...
  async componentDidMount() {
    ...
    this.roomLooper = setInterval(async () => {
      ...
      .set("Accept: application/json")
      const res = await req.send({host: localStorage.getItem("host")});
      if (Object.keys(res.body.overlayImages).length > Object.keys(res.body.players).length) {
        clearInterval(this.roomLooper);
        this.props.onAction("guessScreen1");
      }
    }, 1000);
    if (Object.keys(res.body.overlayImages).length > Object.keys(res.body.players).length) {
      clearInterval(this.roomLooper);
